...
class Encoder:
    def __init__(self, stream_id_iter=_next_stream_id, max_payload=MAX_PAYLOAD_SIZE):
        self.stream_id_iter = stream_id_iter
        self.max_payload = max_payload

    def frames(self, frame_type: int, payload: Any) -> Iterator[bytes]:
        ...
        if payload_len > self.max_payload:
            partial = 1
            stream_id = next(self.stream_id_iter)
        else:
            partial = 0

        while sent_bytes < payload_len:
            chunk_len = min(self.max_payload, payload_len - sent_bytes)
    ...
    def end(self, depth: int) -> Iterator[bytes]:
        ...
        yield from self.frames(END, [depth])  # Version is 1
    ...
    def ping_pong(self) -> Iterator[bytes]:
        ...
        yield from self.frames(PING_PONG, payload)

    def flow_control(self, backoff_value: int) -> Iterator[bytes]:
        payload = {'backoff': backoff_value}
        yield from self.frames(FLOW, payload)

    def error(self, msg: str) -> Iterator[bytes]:
        payload = {'level': 'error', 'text': msg}
        yield from self.frames(LOG, payload)

    def info(self, msg: str) -> Iterator[bytes]:
        payload = {'level': 'info', 'text': msg}
        yield from self.frames(LOG, payload)

    def warning(self, msg: str) -> Iterator[bytes]:
        payload = {'level': 'warning', 'text': msg}
        yield from self.frames(LOG, payload)

    def file(self, file_ref: FileRef, status: str = False) -> Iterator[bytes]:
        """Encodes a file parameter frame."""
        payload = file_ref.model_dump(mode='json')
        payload['status'] = status
        yield from self.frames(FILE, payload)

    def hello(self, client: str, auth_token: Optional[str] = None) -> Iterator[bytes]:
        """Encodes a hello frame with client version."""
        payload = {'ver': 0, 'client': client, 'nonce': randint(0, 2 ** 32)}
        if auth_token:
            payload['auth_token'] = auth_token
        yield from self.frames(HELLO, payload)
