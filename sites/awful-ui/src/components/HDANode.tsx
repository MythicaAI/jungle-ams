...
const HDANode: React.FC<HDANodeProps> = (node) => {
  ...
  const [executionFlowData, setExecutionFlowData] = useState<{ [key: string]: GetFileResponse[] }>({});
  ...
  }, [automationTask.worker, automationTask.path, interfaceFlowData, node.id, runAutomation]);
  ...
  useEffect(() => {
    setExecutionFlowData((prev) => {
      ...
      return JSON.stringify(prev) !== JSON.stringify(rest) ? { ...prev, ...rest } : prev;
    });
    ...
    <div className={`mythica-node worker`}>
      ...
      <div style={{width:'100%'}}>
        ...
        <button 
          onClick={runHda} 
          style={{width:'100%'}} 
          disabled={myExecutionData.state! in [NodeState.Clean, NodeState.Error]}>
          {myExecutionData.state === NodeState.Running ? 'Running...' : ' Run HDA'}
        </button>
      </div>
