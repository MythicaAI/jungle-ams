...
export default function (hou) {
    class _hnt_LOP_timeshift extends hou._HoudiniBase {
        static is_root = false;
        static id = 'LOP/Other/timeshift';
        static category = '/LOP';
        static houdiniType = 'timeshift';
        static title = 'TimeShift';
        static icon = '/editor/assets/imgs/nodes/_hnt_LOP_timeshift.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "inputtimegroup", label: "Houdini Timeline", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
			let hou_parm_template2 = new hou.ToggleParmTemplate({name: "cookinputattime", label: "Cook Input at Specified Time", default_value: true});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "attribtimegroup", label: "USD Time Samples", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "evalattribsattime", label: "Evaluate USD Attributes at Specified Time", default_value: true});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ evalattribsattime == 0 }");
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.StringParmTemplate({name: "proppattern", label: "Properties", num_components: 1, default_value: ["*"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ evalattribsattime == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "getattribdefault", label: "Read Default Values", default_value: false});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ evalattribsattime == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "setattribdefault", label: "Write Default Values", default_value: false});
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ evalattribsattime == 0 }");
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "timegroup", label: "Time Handling", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
    ...
    hou.registerType('LOP/Other/timeshift',_hnt_LOP_timeshift)
    return _hnt_LOP_timeshift
}
