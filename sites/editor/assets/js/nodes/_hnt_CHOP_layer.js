...
export default function (hou) {
    class _hnt_CHOP_layer extends hou.extend(hou._HoudiniBase).with(hou._MultiInputMixin) {
        static is_root = false;
        static id = 'CHOP/Other/layer';
        static category = '/CHOP';
        static houdiniType = 'layer';
        static title = 'Layer';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_layer.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher", label: "Layers", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.ButtonParmTemplate({name: "updatelayernames", label: "Update Layer Names From Inputs"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.IntParmTemplate({name: "active", label: "Active Layer", num_components: 1, default_value: [0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.SeparatorParmTemplate({name: "sep1"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.LabelParmTemplate({name: "warnmsg", label: "Warning Message for Animation Layer workflow", column_labels: ["Warning: Use the Animation Editor ( Animation Layers )\nto add or remove layers. Please refer to the node help."]});
			hou_parm_template2.hideLabel(true);
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FolderParmTemplate({name: "nlayers", label: "Layers", folder_type: hou.folderType.MultiparmBlock, default_value: 1, ends_tab_group: false});
			hou_parm_template2.setTags({"multistartoffset": "0"});
			let hou_parm_template3 = new hou.StringParmTemplate({name: "name#", label: "Name", num_components: 1, default_value: ["layer#"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
			hou_parm_template3.setJoinWithNext(true);
			hou_parm_template2.addParmTemplate(hou_parm_template3);
			hou_parm_template3 = new hou.ToggleParmTemplate({name: "mute#", label: "Mute", default_value: false});
			hou_parm_template2.addParmTemplate(hou_parm_template3);
    ...
    hou.registerType('CHOP/Other/layer',_hnt_CHOP_layer)
    return _hnt_CHOP_layer
}
