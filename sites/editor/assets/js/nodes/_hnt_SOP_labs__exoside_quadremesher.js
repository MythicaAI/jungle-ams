...
export default function (hou) {
    class _hnt_SOP_labs__exoside_quadremesher extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'SOP/Labs/Pipeline/Integration/Exoside Quad Remesher/labs::exoside_quadremesher';
        static category = '/SOP/labs';
        static houdiniType = 'labs::exoside_quadremesher';
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "Quads_sizing", label: "Quads Sizing", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			...
			hou_parm_template2.setHelp("Defines the Color to paint to control locally the desired Quad Density variations (using 'Attribute Paint SOP') \\n . from 0.25 => 'divide density by 4'  =  Big Quads  =  Cyan color \\n . to 4  => 'multiply density by 4'  =  Small Quads  =  Red color.");
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ButtonParmTemplate({name: "SetColor", label: "Adjust 'Attribute Paint SOP'"});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ UseVertexColors == 0 }");
			hou_parm_template2.setHelp("Set the parent 'Attribute Paint SOP' color corresponding with the 'Painted Quad Density' Value\\nand make the Attribute Paint SOP active..\\nThis button is usefull only if the input/parent node of the QuadRemesher node is an Attribute Paint SOP.");
			hou_parm_template2.setScriptCallback("kwargs['node'].hdaModule().buttonSetPaintingColor(kwargs)");
			hou_parm_template2.setScriptCallbackLanguage(hou.scriptLanguage.Python);
			hou_parm_template2.setTags({"export_disable": "1", "script_callback": "kwargs['node'].hdaModule().buttonSetPaintingColor(kwargs)", "script_callback_language": "python"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "Quads_sizing_1", label: "Edge Loops Control", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "Misc", label: "Misc", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
			hou_parm_template2 = new hou.FolderParmTemplate({name: "folder1", label: "Symmetry Axis", folder_type: hou.folderType.Simple, default_value: 0, ends_tab_group: false});
			...
			hou_parm_template3 = new hou.StringParmTemplate({name: "out_cache", label: "Cache File", num_components: 1, default_value: ["$HIP/${HIPNAME}_${OS}_QR_output.fbx"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.FileReference, file_type: hou.fileType.Geometry, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringReplace});
			hou_parm_template2.addParmTemplate(hou_parm_template3);
			hou_parm_template.addParmTemplate(hou_parm_template2);
    ...
    hou.registerType('SOP/Labs/Pipeline/Integration/Exoside Quad Remesher/labs::exoside_quadremesher',_hnt_SOP_labs__exoside_quadremesher)
    return _hnt_SOP_labs__exoside_quadremesher
}
