...
export default function (hou) {
    class _hnt_COP2_function extends hou._HoudiniBase {
        static is_root = false;
        static id = 'COP2/Other/function';
        static category = '/COP2';
        static houdiniType = 'function';
        static title = 'Function';
        static icon = '/editor/assets/imgs/nodes/_hnt_COP2_function.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template2 = new hou.MenuParmTemplate({name: "function", label: "Function", menu_items: ["abs", "sign", "negate", "invert", "floor", "round", "ceil", "sqrt", "powx", "powb", "ex", "log", "ln", "sin", "sin360", "cos", "cos360", "tan", "tan360", "asin", "asin360", "acos", "acos360", "atan", "atan360", "sinh", "sinh360", "cosh", "cosh360", "tanh", "tanh360"], menu_labels: ["Abs(x)", "Sign(x)", "-x", "1/x", "Floor(x)", "Round(x)", "Ceil(x)", "Sqrt(x)", "x^E", "B^x", "e^x", "Log(x)", "Ln(x)", "Sin(x)", "Sin(360x)", "Cos(x)", "Cos(360x)", "Tan(x)", "Tan(360x)", "ASin(x)", "ASin(x) /360", "ACos(x)", "ACos(x) /360", "ATan(x)", "ATan(x) /360", "Sinh(x)", "Sinh(360x)", "Cosh(x)", "Cosh(360x)", "Tanh(x)", "Tanh(360x)"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "base", label: "Base", num_components: 1, default_value: [2], min: 1, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "exp", label: "Exponent", num_components: 1, default_value: [2], min: 1, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.MenuParmTemplate({name: "error", label: "Error Handling", menu_items: ["replace", "keep"], menu_labels: ["Replace With Error Value", "Keep Old Value"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "errval", label: "Error Value", num_components: 1, default_value: [0], min: null, max: 1, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template.addParmTemplate(hou_parm_template2);
    ...
    hou.registerType('COP2/Other/function',_hnt_COP2_function)
    return _hnt_COP2_function
}
