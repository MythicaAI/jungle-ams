...
export default function (hou) {
    class _hnt_CHOP_stashpose extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'CHOP/Character/stashpose';
        static category = '/CHOP';
        static houdiniType = 'stashpose';
        static title = 'Stash Pose';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_stashpose.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "stash", label: "Stash", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.ButtonParmTemplate({name: "updatepose", label: "Update Stash-Pose"});
			hou_parm_template2.setScriptCallback("hou.node(\".\").hdaModule().updatestashpose()");
			hou_parm_template2.setScriptCallbackLanguage(hou.scriptLanguage.Python);
			hou_parm_template2.setTags({"script_callback": "hou.node(\".\").hdaModule().updatestashpose()", "script_callback_language": "python"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "stash_1", label: "Common", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
    ...
    hou.registerType('CHOP/Character/stashpose',_hnt_CHOP_stashpose)
    return _hnt_CHOP_stashpose
}
