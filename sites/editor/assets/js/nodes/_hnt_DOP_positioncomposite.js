...
export default function (hou) {
    class _hnt_DOP_positioncomposite extends hou.extend(hou._HoudiniBase).with(hou._MultiInputMixin) {
        static is_root = false;
        static id = 'DOP/Other/positioncomposite';
        static category = '/DOP';
        static houdiniType = 'positioncomposite';
        static title = 'Position Composite';
        static icon = 'None';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.MenuParmTemplate({name: "parmop_poscompositeop", label: "Composite Type", menu_items: ["initial", "always", "never", "default"], menu_labels: ["![BUTTONS_set_initial]Set Initial", "![BUTTONS_set_or_create]Set Always", "![BUTTONS_set_block]Set Never", "![BUTTONS_set_nothing]Use Default"], default_value: 3, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.ControlNextParameter, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.setTags({"sidefx::look": "icon"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.StringParmTemplate({name: "poscompositeop", label: "Composite Type", num_components: 1, default_value: [""], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: ["first", "inorder", "reverse", "blend"], menu_labels: ["Use First Position", "Apply Positions in Order", "Apply Positions in Reverse Order", "Blend Positions"], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('DOP/Other/positioncomposite',_hnt_DOP_positioncomposite)
    return _hnt_DOP_positioncomposite
}
