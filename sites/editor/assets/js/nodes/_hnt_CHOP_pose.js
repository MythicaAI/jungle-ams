...
export default function (hou) {
    class _hnt_CHOP_pose extends hou._HoudiniBase {
        static is_root = false;
        static id = 'CHOP/Constraints/pose';
        static category = '/CHOP';
        static houdiniType = 'pose';
        static title = 'Pose';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_pose.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.ToggleParmTemplate({name: "__inputs_time_dependent__", label: "Check for Time Dependent Inputs", default_value: false});
			hou_parm_template.hide(true);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher4", label: "Constraint Transform", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			...
			hou_parm_template2.setTags({"autoscope": "1110000000000000"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.MenuParmTemplate({name: "mask", label: "Mask", menu_items: ["1", "2", "4", "8", "16", "32", "64", "128", "256"], menu_labels: ["TX", "TY", "TZ", "RX", "RY", "RZ", "SX", "SY", "SZ"], default_value: 8, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringToggle, menu_use_token: false, is_button_strip: true, strip_uses_icons: false});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ButtonParmTemplate({name: "update", label: "Update Pose"});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(0) == 0 }");
			hou_parm_template2.setScriptCallback("import choptoolutils; reload(choptoolutils); choptoolutils.updatePose(kwargs['node'])");
			hou_parm_template2.setScriptCallbackLanguage(hou.scriptLanguage.Python);
			hou_parm_template2.setTags({"script_callback": "import choptoolutils; reload(choptoolutils); choptoolutils.updatePose(kwargs['node'])", "script_callback_language": "python"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ButtonParmTemplate({name: "clear", label: "Clear Pose"});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(0) == 0 }");
			hou_parm_template2.setScriptCallback("import choptoolutils; reload(choptoolutils); choptoolutils.clearPose(kwargs['node'])");
			hou_parm_template2.setScriptCallbackLanguage(hou.scriptLanguage.Python);
			hou_parm_template2.setTags({"script_callback": "import choptoolutils; reload(choptoolutils); choptoolutils.clearPose(kwargs['node'])", "script_callback_language": "python"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('CHOP/Constraints/pose',_hnt_CHOP_pose)
    return _hnt_CHOP_pose
}
