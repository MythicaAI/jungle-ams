...
export default function (hou) {
    class _hnt_VOP_pxrdistantlight extends hou._HoudiniBase {
        static is_root = false;
        static id = 'VOP/Other/pxrdistantlight';
        static category = '/VOP';
        static houdiniType = 'pxrdistantlight';
        static title = 'Pxr Distant Light';
        static icon = '/editor/assets/imgs/nodes/_hnt_VOP_pxrdistantlight.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template2 = new hou.FloatParmTemplate({name: "intensity", label: "Intensity", num_components: 1, default_value: [50000], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setHelp("intensity: Specify the intensity of the light source linearly. The light intensity is computed using physical units. The default light begins at a high value of 50000.0.");
			...
			hou_parm_template2.setTags({"script_ritype": "float"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "angleExtent", label: "Angle Extent", num_components: 1, default_value: [0.53], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setHelp("angleExtent: Opening angle of the light source in degrees (Sun=0.53). Higher values soften the shadow edges (light rays become less parallel) while lower values make it sharper. Note that increasing this parameter also increases the amount of light in the scene and you may need to significantly reduce the light intensity/exposure to compensate or you may use Area Normalize to prevent changes in illumination.");
			hou_parm_template2.setTags({"script_ritype": "float"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template2.setHelp("enableTemperature: Turns the color temperature on or off.");
			...
			hou_parm_template2.setHelp("emissionFocus: Off-axis cosine power exponent to shape the emission. This controls the spread of the light. Higher number will start focusing the light towards the center and thus narrowing the light spread. For the PxrDistantLight it will appear to dim the lighting as you increase the parameter. To alter the shadowing to appear larger/softer you should use Angle Extent.");
			hou_parm_template2.setTags({"script_ritype": "float"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template2.setHelp("When normalize is enabled, the amount of light contributed to the scene will not change as you change the angleExtent of the light source. This makes it easier to adjust highlight size without changing scene lighting.");
			hou_parm_template2.setTags({"script_ritype": "int"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
    ...
    hou.registerType('VOP/Other/pxrdistantlight',_hnt_VOP_pxrdistantlight)
    return _hnt_VOP_pxrdistantlight
}
