...
export default function (hou) {
    class _hnt_VOP_ocio_transform extends hou._HoudiniBase {
        static is_root = false;
        static id = 'VOP/Other/ocio_transform';
        static category = '/VOP';
        static houdiniType = 'ocio_transform';
        static title = 'OCIO Transform';
        static icon = '/editor/assets/imgs/nodes/_hnt_VOP_ocio_transform.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.StringParmTemplate({name: "tospace", label: "To Space", num_components: 1, default_value: ["scene_linear"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "echo `pythonexprs(\"__import__(\'toolutils\').ocioColorSpaceMenu()\")`\n", item_generator_script_language: hou.scriptLanguage.Hscript, menu_type: hou.menuType.StringReplace});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('VOP/Other/ocio_transform',_hnt_VOP_ocio_transform)
    return _hnt_VOP_ocio_transform
}
