...
export default function (hou) {
    class _hnt_SOP_groupbylasso extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'SOP/Group/groupbylasso';
        static category = '/SOP';
        static houdiniType = 'groupbylasso';
        static title = 'Group by Lasso';
        static icon = '/editor/assets/imgs/nodes/_hnt_SOP_groupbylasso.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.StringParmTemplate({name: "group", label: "Group Name", num_components: 1, default_value: ["group"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "geoType = kwargs['node'].parm('grouptype')\ntype = geoType.eval()\n\ngeo = hou.node('./null1').geometry()\ngroups = []\n\nif geo:\n    if type == 0: # Points\n        groups = geo.pointGroups()\n    elif type == 1: # Prims\n        groups = geo.primGroups()\n\nmenu = []\nfor group in groups:\n    menu.append(group.name())\n    menu.append(group.name())\n\nreturn menu", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringReplace});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "grouptype", label: "Group Type", menu_items: ["points", "primitives"], menu_labels: ["Points", "Primitives"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "onlyfull", label: "Only Promote Full Primitives", default_value: false});
			hou_parm_template.setConditional(hou.parmCondType.HideWhen, "{ grouptype != primitives }");
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "combine", label: "Combine Operation", menu_items: ["replace", "union", "intersection", "xor", "subtract"], menu_labels: ["Replace", "Union", "Intersect", "Exclusive Or", "Subtract"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "displayprojected", label: "Display Projected Regions as Guides", default_value: false});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "folder0", label: "Stroke", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.FloatParmTemplate({name: "stroke_radius", label: "Radius", num_components: 1, default_value: [0.1], min: 0, max: 1, min_is_strict: true, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			...
			hou_parm_template2.setTags({"autoscope": "0000000000000000"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "stroke_visibleonly", label: "Select Only Visible", default_value: true});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "stroke_bias", label: "Visibility Bias", num_components: 1, default_value: [0.05], min: 0.001, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Logarithmic, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.HideWhen, "{ stroke_visibleonly == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template3 = new hou.FloatParmTemplate({name: "stroke#_radius", label: "Radius", num_components: 1, default_value: [1], min: 0, max: 1, min_is_strict: true, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
    ...
    hou.registerType('SOP/Group/groupbylasso',_hnt_SOP_groupbylasso)
    return _hnt_SOP_groupbylasso
}
