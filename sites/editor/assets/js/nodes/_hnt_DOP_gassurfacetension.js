...
export default function (hou) {
    class _hnt_DOP_gassurfacetension extends hou.extend(hou._HoudiniBase).with(hou._MultiInputMixin) {
        static is_root = false;
        static id = 'DOP/Other/gassurfacetension';
        static category = '/DOP';
        static houdiniType = 'gassurfacetension';
        static title = 'Gas Surface Tension';
        static icon = '/editor/assets/imgs/nodes/_hnt_DOP_gassurfacetension.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "parmop_surfacetension", label: "Surface Tension", menu_items: ["initial", "always", "never", "default"], menu_labels: ["![BUTTONS_set_initial]Set Initial", "![BUTTONS_set_or_create]Set Always", "![BUTTONS_set_block]Set Never", "![BUTTONS_set_nothing]Use Default"], default_value: 3, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.ControlNextParameter, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.setTags({"sidefx::look": "icon"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FloatParmTemplate({name: "surfacetension", label: "Surface Tension", num_components: 1, default_value: [0.1], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
    ...
    hou.registerType('DOP/Other/gassurfacetension',_hnt_DOP_gassurfacetension)
    return _hnt_DOP_gassurfacetension
}
