...
export default function (hou) {
    class _hnt_DOP_popkill extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'DOP/POPs/popkill';
        static category = '/DOP';
        static houdiniType = 'popkill';
        static title = 'POP Kill';
        static icon = '/editor/assets/imgs/nodes/_hnt_DOP_popkill.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "showguide", label: "Label", default_value: true});
			hou_parm_template.hideLabel(true);
			hou_parm_template.setJoinWithNext(true);
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template2 = new hou.StringParmTemplate({name: "rulecode", label: "VEXpression", num_components: 1, default_value: ["dead = 1;"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "import vexpressionmenu\n\nreturn vexpressionmenu.buildSnippetMenu('popkill/rulecode')", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringReplace});
			...
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ boundtype == geometry } { boundtype == volume }");
			...
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ boundtype == sphere } { boundtype == box }");
			...
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ boundtype != volume }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "enablerandom", label: "Enable Random", default_value: false, default_expression: "off", default_expression_language: hou.scriptLanguage.Hscript});
			hou_parm_template2.setTags({"autoscope": "0000000000000000", "parmvop": "1", "shaderparmcontexts": "cvex"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.MenuParmTemplate({name: "randombehavior", label: "Behavior", menu_items: ["add", "remove"], menu_labels: ["Add to Kill List", "Remove from Kill List"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
    ...
    hou.registerType('DOP/POPs/popkill',_hnt_DOP_popkill)
    return _hnt_DOP_popkill
}
