...
export default function (hou) {
    class _hnt_SOP_labs__mapbox__2_0 extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'SOP/Labs/Import/labs::mapbox::2.0';
        static category = '/SOP/labs';
        static houdiniType = 'labs::mapbox::2.0';
        static title = 'Labs MapBox';
        static icon = '/editor/assets/imgs/nodes/_hnt_SOP_labs__mapbox__2_0.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "folder1", label: "Base Settings", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "folder1_1", label: "OSM Settings", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
			...
			hou_parm_template2.setHelp("Vertical offset applied to OSM data.");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "snap_to_terrain", label: "Snap To Terrain", default_value: true});
			hou_parm_template2.setHelp("Project OSM data onto the heightfield, when this toggle is disabled the OSM data will be flat.");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "height_from_terrain", label: "Building Height From Terrain", default_value: false});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ snap_to_terrain == 1 }");
			hou_parm_template2.setHelp("Set a height attribute if it exists in the OSM data (otherwise calculate it from projecting onto the terrain).");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "color_from_terrain", label: "Color From Terrain", default_value: true});
			hou_parm_template2.setHelp("Use the downloaded colour map to set a primitive colour for the buildings.");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "resample_curve", label: "Resample OSM Curves", default_value: false});
			hou_parm_template2.setHelp("Enable extra points to be inserted into the building curves.");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "segment_length", label: "Segment Length", num_components: 1, default_value: [1], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ resample_curve == 0 }");
			hou_parm_template2.setHelp("Length used to generate extra curve points for the buildings.");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "folder1_2", label: "Local Download Paths", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			hou_parm_template.setTags({"group_type": "simple"});
    ...
    hou.registerType('SOP/Labs/Import/labs::mapbox::2.0',_hnt_SOP_labs__mapbox__2_0)
    return _hnt_SOP_labs__mapbox__2_0
}
