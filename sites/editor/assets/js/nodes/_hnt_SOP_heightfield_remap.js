...
export default function (hou) {
    class _hnt_SOP_heightfield_remap extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'SOP/Environments/Terrain/heightfield_remap';
        static category = '/SOP';
        static houdiniType = 'heightfield_remap';
        static title = 'HeightField Remap';
        static icon = '/editor/assets/imgs/nodes/_hnt_SOP_heightfield_remap.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.StringParmTemplate({name: "layer", label: "Layer to Remap", num_components: 1, default_value: ["height"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "import terraintoolutils \n\nreturn terraintoolutils.buildNameMenu(kwargs['node'])", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringReplace});
			...
			hou_parm_template.setTags({"autoscope": "0000000000000000"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "clampmin", label: "Clamp to Minimum", default_value: false});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "clampmax", label: "Clamp to Maximum", default_value: false});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('SOP/Environments/Terrain/heightfield_remap',_hnt_SOP_heightfield_remap)
    return _hnt_SOP_heightfield_remap
}
