...
export default function (hou) {
    class _hnt_SOP_distancefromgeometry extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'SOP/Geometry/Analysis/distancefromgeometry';
        static category = '/SOP';
        static houdiniType = 'distancefromgeometry';
        static title = 'Distance from Geometry';
        static icon = '/editor/assets/imgs/nodes/_hnt_SOP_distancefromgeometry.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.StringParmTemplate({name: "refgroup", label: "Reference Group", num_components: 1, default_value: [""], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "kwargs[\'node\'].generateInputGroupMenu(1, hou.geometryType.Points if kwargs[\'node\'].parm(\'refgrouptype\').evalAsString() == \"points\" else hou.geometryType.Primitives)", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringToggle});
			hou_parm_template.setTags({"autoscope": "0000000000000000", "script_action": "import soputils\nkwargs['geometrytype'] = kwargs['node'].parmTuple('refgrouptype')\nkwargs['inputindex'] = 1\nsoputils.selectGroupParm(kwargs)", "script_action_help": "Select geometry from an available viewport.", "script_action_icon": "BUTTONS_reselect"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "refgrouptype", label: "Reference Group Type", menu_items: ["prims", "points"], menu_labels: ["Primitives", "Points"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.setTags({"autoscope": "0000000000000000"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "distmetric", label: "Distance Metric", menu_items: ["surface", "inside", "outside"], menu_labels: ["Distance from Surface", "Distance Inside Geometry", "Distance Outside Geometry"], default_value: 2, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ refgrouptype != prims }");
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('SOP/Geometry/Analysis/distancefromgeometry',_hnt_SOP_distancefromgeometry)
    return _hnt_SOP_distancefromgeometry
}
