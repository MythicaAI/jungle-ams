...
export default function (hou) {
    class _hnt_CHOP_posedifference extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'CHOP/Character/posedifference';
        static category = '/CHOP';
        static houdiniType = 'posedifference';
        static title = 'Pose Difference';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_posedifference.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "difference", label: "Difference", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.FloatParmTemplate({name: "referencepose", label: "Reference Pose", num_components: 1, default_value: [1], min: null, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(1) == 1 }");
			hou_parm_template2.setTags({"autoscope": "0000000000000000"});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "difference_1", label: "Common", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
    ...
    hou.registerType('CHOP/Character/posedifference',_hnt_CHOP_posedifference)
    return _hnt_CHOP_posedifference
}
