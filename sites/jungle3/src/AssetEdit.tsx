...
import {LucideCircleMinus, LucideEllipsis, LucideSidebarClose, LucideUndo, LucideXSquare} from "lucide-react";
...
export const AssetEdit: React.FC<AssetEditProps> = ({prop_asset_id=undefined, prop_version="0.0.0"}) => {
  ...
  const onUpdateOrg = (event, value: string) => {
  ...
  const onVersionBlur = (event) => {
    ...
    const parts = versionStr
        ...
        .map(v => parseInt(v, 10))
        .filter(v => !isNaN(v));
    event.target.value = sanitizeVersion(parts).join('.')
  }
