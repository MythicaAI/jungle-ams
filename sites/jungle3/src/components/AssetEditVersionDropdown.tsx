...
import {
    ...
    FormControl,
    List,
    Card, ListItemButton, ListItem, ListItemDecorator, Switch, Typography, Stack
} from '@mui/joy';
...
export const AssetEditVersionDropdown = () => {
    ...
    const [availableVersions, setAvailableVersions] = useState<AssetVersionResponse[]>([]);
    ...
    useEffect( () => {
        if (asset_id) {
            getData<AssetVersionResponse[]>(`assets/${asset_id}`).then(r => {
    ...
    return <FormControl error={isVersionZero(sanitizedVersion)}>
        ...
        <Stack direction={"row"} alignItems={"center"}>
            ...
            <Popper open={open} placement="bottom-start" anchorEl={anchorEl}>
                <ClickAwayListener onClickAway={() => allowClickAway && setAnchorEl(null) }>
                <Card>
                    <List>
                        {availableVersions.map((av, index) => (
                            <ListItem key={index}>
                                <ListItemButton
                                    onClick={() => {
                                        setAnchorEl(null);
                                        navigate(`/assets/${asset_id}/versions/${av.version.join('.')}`)
                                    }}>
                                    {av.version.join('.')}
                                </ListItemButton>
                                <ListItemDecorator>
