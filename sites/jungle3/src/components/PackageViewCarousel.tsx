...
import {Box, IconButton, AspectRatio, Skeleton, Divider} from '@mui/joy';
 ...
 const assetContentToImageRef = (avc: AssetVersionContent): ImageRef => {
  ...
  const getAssetVersionThumbnails = (av: AssetVersionResponse): ImageRef[] => {
    if (av && 'contents' in av && 'thumbnails' in av.contents) {
      return av.contents['thumbnails'].map((content) => assetContentToImageRef(content));
    }
    return [];
  }
...
const PackageViewCarousel: React.FC<AssetVersionResponse> = (av) => {
  ...
  return thumbnailRefs.length == 0 ? <Skeleton animation={false} width={800} height={600} /> :
    <Box sx={{ width: '100%', maxWidth: 600, margin: 'auto', position: 'relative' }}>
      ...
      <Divider orientation="horizontal" sx={{ position: 'relative'}} />
