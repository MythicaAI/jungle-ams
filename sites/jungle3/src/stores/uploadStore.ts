...
const addUpload = (uploads: FileUploadMap, upload: FileUploadStatus): FileUploadMap => {
    return {
        ...uploads,
...
const addUploads = (uploads: FileUploadMap, newUploads: FileUploadStatus[]): FileUploadMap => {
    return {
        ...uploads,
...
const updateUpload = (uploads: FileUploadMap, file_id: string, progress: number): FileUploadMap => {
...
export const useUploadStore = create<UploadState>((set) => ({
    ...
    setUploadProgress: (progress: number) => set((state) => {
        ...
        return ({progress: progress, progressState: progressState, pendingUploads: pendingUploads})
    }),
    trackUpload: (file: FileUploadStatus) => set((state) =>
        ({uploads: addUpload(state.uploads, file)})),
    trackUploads: (files: FileUploadStatus[]) => set((state) =>
        ({uploads: addUploads(state.uploads, files)})),
    updateUpload: (file_id: string, progress: number) => set((state) =>
