...
export default function (hou) {
    class _hnt_OBJECT_pxrintmultlightfilter extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'OBJECT/Render/pxrintmultlightfilter';
        static category = '/OBJECT';
        static houdiniType = 'pxrintmultlightfilter';
        static title = 'Pxr Intensity Mult Light Filter';
        static icon = '/editor/assets/imgs/nodes/_hnt_OBJECT_pxrintmultlightfilter.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template2 = new hou.FolderParmTemplate({name: "multiplier", label: "Multiplier", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template3 = new hou.FloatParmTemplate({name: "intensity", label: "Intensity", num_components: 1, default_value: [1], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template3.setHelp("intensity: Multiplier for the light intensity.");
			...
			hou_parm_template3.setHelp("exposure: Exposure control for the multiplier.");
			hou_parm_template3.setTags({"autoscope": "0000000000000000", "script_ritype": "float"});
			hou_parm_template2.addParmTemplate(hou_parm_template3);
			...
			hou_parm_template3.setHelp("invert: When this is on, invert the multiplier. For instance, if we set the light filter's intensity to 2, the final light's intensity will be divided by (instead of multiplied by) the light filter's intensity.");
    ...
    hou.registerType('OBJECT/Render/pxrintmultlightfilter',_hnt_OBJECT_pxrintmultlightfilter)
    return _hnt_OBJECT_pxrintmultlightfilter
}
