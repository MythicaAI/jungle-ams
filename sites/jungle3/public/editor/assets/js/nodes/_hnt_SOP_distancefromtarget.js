...
export default function (hou) {
    class _hnt_SOP_distancefromtarget extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'SOP/Geometry/Analysis/distancefromtarget';
        static category = '/SOP';
        static houdiniType = 'distancefromtarget';
        static title = 'Distance from Target';
        static icon = '/editor/assets/imgs/nodes/_hnt_SOP_distancefromtarget.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "target", label: "Target", menu_items: ["point", "line", "plane"], menu_labels: ["Point", "Line", "Plane"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template.setTags({"autoscope": "0000000000000000"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FloatParmTemplate({name: "dir", label: "Direction", num_components: 3, default_value: [1, 0, 0], min: null, max: 1, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.XYZW});
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ target == radial }");
			hou_parm_template.setTags({"autoscope": "0000000000000000"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "distmetric", label: "Distance Metric", menu_items: ["abs", "signed"], menu_labels: ["Absolute Distance", "Signed Distance"], default_value: 1, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ target != plane }");
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('SOP/Geometry/Analysis/distancefromtarget',_hnt_SOP_distancefromtarget)
    return _hnt_SOP_distancefromtarget
}
