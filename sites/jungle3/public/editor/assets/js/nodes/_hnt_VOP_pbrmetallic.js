...
export default function (hou) {
    class _hnt_VOP_pbrmetallic extends hou._HoudiniBase {
        static is_root = false;
        static id = 'VOP/Shading (BSDFs)/pbrmetallic';
        static category = '/VOP';
        static houdiniType = 'pbrmetallic';
        static title = 'PBR Metallic Reflection';
        static icon = '/editor/assets/imgs/nodes/_hnt_VOP_pbrmetallic.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template = new hou.StringParmTemplate({name: "label", label: "Component Label", num_components: 1, default_value: ["reflect"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: ["diffuse", "specular", "reflect"], menu_labels: ["Diffuse", "Specular", "Reflect"], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.StringReplace});
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ model == cone }");
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "frontface", label: "Shade Both Sides As Front", default_value: true});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('VOP/Shading (BSDFs)/pbrmetallic',_hnt_VOP_pbrmetallic)
    return _hnt_VOP_pbrmetallic
}
