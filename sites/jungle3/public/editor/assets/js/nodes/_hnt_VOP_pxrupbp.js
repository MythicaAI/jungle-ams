...
export default function (hou) {
    class _hnt_VOP_pxrupbp extends hou._HoudiniBase {
        static is_root = false;
        static id = 'VOP/Other/pxrupbp';
        static category = '/VOP';
        static houdiniType = 'pxrupbp';
        static title = 'Pxr UPBP';
        static icon = '/editor/assets/imgs/nodes/_hnt_VOP_pxrupbp.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template.setHelp("When on, bidirectional path tracing is enabled; otherwise, UPBP will operate as a forward path tracer (if merging is also disabled). Recommended for most scenes, especially interior scenes with significant indirect illumination. On by default.");
			...
			hou_parm_template.setTags({"script_ritype": "point"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FloatParmTemplate({name: "beamsPathsRatio", label: "beamsPathsRatio", num_components: 1, default_value: [0.05], min: 0, max: 1, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template.setHelp("Specifies how large a fraction of the UPBP light paths travelling through a volume should be stored as photon beams. (The other light paths will be stored as photon points.) For sparse volumes more beams give less noise, but they also increase render time. The default value is 0.05 (i.e. five percent).");
			hou_parm_template.setTags({"script_ritype": "float"});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('VOP/Other/pxrupbp',_hnt_VOP_pxrupbp)
    return _hnt_VOP_pxrupbp
}
