...
export default function (hou) {
    class _hnt_DOP_gasanalysis extends hou.extend(hou._HoudiniBase).with(hou._MultiInputMixin) {
        static is_root = false;
        static id = 'DOP/Other/gasanalysis';
        static category = '/DOP';
        static houdiniType = 'gasanalysis';
        static title = 'Gas Analysis';
        static icon = '/editor/assets/imgs/nodes/_hnt_DOP_gasanalysis.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ opencl == 1 analysis != curvature analysis != laplacian }");
			hou_parm_template.setTags({"sidefx::look": "icon"});
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ opencl == 1 analysis != curvature analysis != laplacian }");
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "parmop_analysis", label: "Analysis", menu_items: ["initial", "always", "never", "default"], menu_labels: ["![BUTTONS_set_initial]Set Initial", "![BUTTONS_set_or_create]Set Always", "![BUTTONS_set_block]Set Never", "![BUTTONS_set_nothing]Use Default"], default_value: 3, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.ControlNextParameter, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template.setTags({"sidefx::look": "icon"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.MenuParmTemplate({name: "analysis", label: "Analysis", menu_items: ["curvature", "gradient", "laplacian", "divergence", "curl", "normalize", "length"], menu_labels: ["Curvature", "Gradient", "Laplacian", "Divergence", "Curl", "Normalize", "Length"], default_value: 0, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ analysis == curvature } { analysis == laplacian }");
			hou_parm_template.setTags({"sidefx::look": "icon"});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('DOP/Other/gasanalysis',_hnt_DOP_gasanalysis)
    return _hnt_DOP_gasanalysis
}
