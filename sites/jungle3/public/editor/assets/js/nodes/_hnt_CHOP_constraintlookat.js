...
export default function (hou) {
    class _hnt_CHOP_constraintlookat extends hou._HoudiniBase {
        static is_root = false;
        static id = 'CHOP/Constraints/constraintlookat';
        static category = '/CHOP';
        static houdiniType = 'constraintlookat';
        static title = 'Look At';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_constraintlookat.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher4", label: "Constraint Lookat", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "lookat", label: "Look At Position", num_components: 3, default_value: [0, 0, 0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.XYZW});
			hou_parm_template2.setConditional(hou.parmCondType.HideWhen, "{ hasinput(1) == 1 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.MenuParmTemplate({name: "mode", label: "Look Up Mode", menu_items: ["0", "1", "2"], menu_labels: ["Use Position", "Use Vector", "Use Quaternion"], default_value: 1, icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false, is_button_strip: false, strip_uses_icons: false});
			hou_parm_template2.setConditional(hou.parmCondType.HideWhen, "{ hasinput(2) == 1 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "uppos", label: "Look Up Position", num_components: 3, default_value: [0, 0, 1], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.XYZW});
			hou_parm_template2.setConditional(hou.parmCondType.HideWhen, "{ hasinput(1) == 1 } { mode == 2 } { mode == 1 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "upvec", label: "Look Up Vector", num_components: 3, default_value: [0, 1, 0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.XYZW});
			hou_parm_template2.setConditional(hou.parmCondType.HideWhen, "{ hasinput(2) == 1 } { mode == 2 } { mode == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "twist", label: "Twist", num_components: 1, default_value: [0], min: null, max: 180, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Angle, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.HideWhen, "{ hasinput(3) == 1 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
    ...
    hou.registerType('CHOP/Constraints/constraintlookat',_hnt_CHOP_constraintlookat)
    return _hnt_CHOP_constraintlookat
}
