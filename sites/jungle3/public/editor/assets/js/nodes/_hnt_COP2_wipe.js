...
export default function (hou) {
    class _hnt_COP2_wipe extends hou._HoudiniBase {
        static is_root = false;
        static id = 'COP2/Other/wipe';
        static category = '/COP2';
        static houdiniType = 'wipe';
        static title = 'Wipe';
        static icon = '/editor/assets/imgs/nodes/_hnt_COP2_wipe.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher", label: "Wipe", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.StringParmTemplate({name: "op", label: "Operation", num_components: 1, default_value: ["cross"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: ["cross", "dissolve", "vwipe", "hwipe", "shrink", "circle", "AAcircle"], menu_labels: ["Cross Dissolve", "Per-Pixel Flip", "Vertical Wipe", "Horizontal Wipe", "4 Corner Shrink", "Circular Shrink", "AA Circular Shrink"], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "level", label: "Amount", num_components: 1, default_value: [0.5], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "doline", label: "Draw line", default_value: true});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ op == dissolve }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "lwidth", label: "Line Width", num_components: 1, default_value: [0.02], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ op == dissolve }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "lcol", label: "Line Colour", num_components: 4, default_value: [1, 1, 1, 1], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.ColorSquare, naming_scheme: hou.parmNamingScheme.RGBA});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ op == dissolve }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
    ...
    hou.registerType('COP2/Other/wipe',_hnt_COP2_wipe)
    return _hnt_COP2_wipe
}
