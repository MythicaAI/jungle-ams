...
export default function (hou) {
    class _hnt_OBJECT_switcher extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin, hou._MultiInputMixin) {
        static is_root = false;
        static id = 'OBJECT/Other/switcher';
        static category = '/OBJECT';
        static houdiniType = 'switcher';
        static title = 'Switcher';
        static icon = '/editor/assets/imgs/nodes/_hnt_OBJECT_switcher.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template2.hide(true);
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.IntParmTemplate({name: "camswitch", label: "Switch Camera", num_components: 1, default_value: [0], min: 0, max: 10, min_is_strict: true, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false});
			...
			hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher_1", label: "Misc", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
    ...
    hou.registerType('OBJECT/Other/switcher',_hnt_OBJECT_switcher)
    return _hnt_OBJECT_switcher
}
