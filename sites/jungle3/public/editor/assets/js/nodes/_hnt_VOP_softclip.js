...
export default function (hou) {
    class _hnt_VOP_softclip extends hou._HoudiniBase {
        static is_root = false;
        static id = 'VOP/Utility/softclip';
        static category = '/VOP';
        static houdiniType = 'softclip';
        static title = 'Soft Clip';
        static icon = 'None';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "opsettings", label: "VOP Parameters", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.ToggleParmTemplate({name: "active", label: "Active", default_value: true});
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "start", label: "Start", num_components: 1, default_value: [0.8], min: 0, max: 1, min_is_strict: true, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ active == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.FloatParmTemplate({name: "sup", label: "Suppress", num_components: 1, default_value: [1], min: 0, max: 10, min_is_strict: true, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ active == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FolderParmTemplate({name: "opsettings_1", label: "UI Promotion", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
    ...
    hou.registerType('VOP/Utility/softclip',_hnt_VOP_softclip)
    return _hnt_VOP_softclip
}
