...
export default function (hou) {
    class _hnt_CHOP_constrainttransform extends hou._HoudiniBase {
        static is_root = false;
        static id = 'CHOP/Constraints/constrainttransform';
        static category = '/CHOP';
        static houdiniType = 'constrainttransform';
        static title = 'Transform';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_constrainttransform.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher4", label: "Constraint Transform", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			...
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(2) == 1 pmode != 1 } { hasinput(1) == 1 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			...
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(2) == 1 pmode != 0 } { hasinput(1) == 1 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.IntParmTemplate({name: "mode", label: "Mode", num_components: 1, default_value: [0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1, menu_items: ["0", "1", "2", "3"], menu_labels: ["Post-Multiply First Input", "Pre-Multiply First Input", "Return First Input", "Return Second Input"], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(0) == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.IntParmTemplate({name: "pmode", label: "Pivot Mode", num_components: 1, default_value: [0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1, menu_items: ["0", "1", "2"], menu_labels: ["Pivot from Input#3. Pivot Rotate from Parameters", "Pivot from Parameters. Pivot Rotate from Input #3", "Both from Input #3"], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal, menu_use_token: false});
			hou_parm_template2.setConditional(hou.parmCondType.DisableWhen, "{ hasinput(2) == 0 }");
			hou_parm_template.addParmTemplate(hou_parm_template2);
			hou_parm_template2 = new hou.ToggleParmTemplate({name: "invert", label: "Invert Transform", default_value: false});
			hou_parm_template.addParmTemplate(hou_parm_template2);
    ...
    hou.registerType('CHOP/Constraints/constrainttransform',_hnt_CHOP_constrainttransform)
    return _hnt_CHOP_constrainttransform
}
