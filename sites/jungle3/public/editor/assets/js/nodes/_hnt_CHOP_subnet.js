...
export default function (hou) {
    class _hnt_CHOP_subnet extends hou.extend(hou._HoudiniBase).with(hou._SubgraphMixin) {
        static is_root = false;
        static id = 'CHOP/Other/subnet';
        static category = '/CHOP';
        static houdiniType = 'subnet';
        static title = 'Subnetwork';
        static icon = '/editor/assets/imgs/nodes/_hnt_CHOP_subnet.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.FolderParmTemplate({name: "stdswitcher", label: "SubNet", folder_type: hou.folderType.Tabs, default_value: 0, ends_tab_group: false});
			let hou_parm_template2 = new hou.StringParmTemplate({name: "label1", label: "Input #1 Label", num_components: 1, default_value: ["Sub-Network Input #1"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
    ...
    hou.registerType('CHOP/Other/subnet',_hnt_CHOP_subnet)
    return _hnt_CHOP_subnet
}
