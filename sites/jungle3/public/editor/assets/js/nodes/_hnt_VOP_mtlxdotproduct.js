...
export default function (hou) {
    class _hnt_VOP_mtlxdotproduct extends hou._HoudiniBase {
        static is_root = false;
        static id = 'VOP/MaterialX/Math/mtlxdotproduct';
        static category = '/VOP';
        static houdiniType = 'mtlxdotproduct';
        static title = 'MtlX Dot Product';
        static icon = '/editor/assets/imgs/nodes/_hnt_VOP_mtlxdotproduct.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			hou_parm_template = new hou.FloatParmTemplate({name: "in1", label: "Input 1", num_components: 2, default_value: [0, 0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.XYZW});
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template.setTags({"sidefx::shader_parmname": "in1"});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FloatParmTemplate({name: "in2", label: "Input 2", num_components: 2, default_value: [0, 0], min: 0, max: 10, min_is_strict: false, max_is_strict: false, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.XYZW});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('VOP/MaterialX/Math/mtlxdotproduct',_hnt_VOP_mtlxdotproduct)
    return _hnt_VOP_mtlxdotproduct
}
