...
export default function (hou) {
    class _hnt_DOP_gasbuildcollisionmaskfrompieces extends hou.extend(hou._HoudiniBase).with(hou._MultiInputMixin) {
        static is_root = false;
        static id = 'DOP/Other/gasbuildcollisionmaskfrompieces';
        static category = '/DOP';
        static houdiniType = 'gasbuildcollisionmaskfrompieces';
        static title = 'Gas Build Collision Mask From Pieces';
        static icon = '/editor/assets/imgs/nodes/_hnt_DOP_gasbuildcollisionmaskfrompieces.svg';
        constructor() {
            super();
        ...
        parmTemplatesInit() {
			...
			let hou_parm_template = new hou.StringParmTemplate({name: "collision", label: "Collision Field", num_components: 1, default_value: ["collision"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.ToggleParmTemplate({name: "clearmask", label: "Clear Mask", default_value: true});
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.FloatParmTemplate({name: "minmaskval", label: "Starting Value", num_components: 1, default_value: [null], min: null, max: 0, min_is_strict: false, max_is_strict: true, look: hou.parmLook.Regular, naming_scheme: hou.parmNamingScheme.Base1});
			hou_parm_template.setConditional(hou.parmCondType.DisableWhen, "{ clearmask == 0 }");
			hou_parm_template_group.append(hou_parm_template);
			...
			hou_parm_template_group.append(hou_parm_template);
			hou_parm_template = new hou.StringParmTemplate({name: "surfvol", label: "Surface Volume", num_components: 1, default_value: ["surface"], naming_scheme: hou.parmNamingScheme.Base1, string_type: hou.stringParmType.Regular, menu_items: [], menu_labels: [], icon_names: [], item_generator_script: "", item_generator_script_language: hou.scriptLanguage.Python, menu_type: hou.menuType.Normal});
			hou_parm_template_group.append(hou_parm_template);
    ...
    hou.registerType('DOP/Other/gasbuildcollisionmaskfrompieces',_hnt_DOP_gasbuildcollisionmaskfrompieces)
    return _hnt_DOP_gasbuildcollisionmaskfrompieces
}
