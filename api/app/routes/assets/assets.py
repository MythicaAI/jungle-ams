...
@router.get('/all')
async def list_all() -> list[AssetVersionResult]:
...
@router.get('/top')
async def list_top() -> list[AssetTopResult]:
...
async def named(asset_name) -> list[AssetVersionResult]:
    """Get asset by name"""
    with get_session() as session:
...
async def committed_at(ref: str) -> list[AssetVersionResult]:
...
@router.get('/{asset_id}')
async def by_id(asset_id: str) -> list[AssetVersionResult]:
    """Get asset by id"""
    with get_session() as session:
...
async def create(r: AssetCreateRequest,
...
async def delete_version(asset_id: str, version_str: str, profile: Profile = Depends(current_profile)):
