...
async def complete_email(
    ...
    validate_result = await db_session.exec(
        select(Profile).where(Profile.profile_seq == profile.profile_seq))
    validate_profile = validate_result.first()
    ...
    validate_key = validate_key_result.first()
    ...
    await db_session.exec(delete(ProfileKey).where(
        ProfileKey.key == verification_code
    ))
