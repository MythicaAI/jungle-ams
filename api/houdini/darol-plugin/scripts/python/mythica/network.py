    ...
    if content is None:
        ...
        try:
            if binary:
                ...
                content.save(os.path.join(icon_path,icon))
    ...
    if isinstance(obj, hou.Geometry):
        ...
        for attrib in obj.pointAttribs():
            ...
            if attrib.dataType() == hou.attribData.String:
                values = np.array(obj.pointStringAttribValues(attribName))
            elif attrib.dataType() == hou.attribData.Float:
                values = np.array(obj.pointFloatAttribValues(attribName))
            elif attrib.dataType() == hou.attribData.Int:
                values = np.array(obj.pointIntAttribValues(attribName))
            
            if len(values) > 0: 
                if attrib.size() > 1:
                    values = values.reshape(-1,attrib.size())
                data['points'][attrib.name()] = values.tolist()
        ...
        for attrib in obj.primAttribs():
            ...
            values = None
            if attrib.dataType() == hou.attribData.String:
                values = np.array(obj.primStringAttribValues(attribName))
            elif attrib.dataType() == hou.attribData.Float:
                values = np.array(obj.primFloatAttribValues(attribName))
            elif attrib.dataType() == hou.attribData.Int:
                values = np.array(obj.primIntAttribValues(attribName))
            
            if len(values) > 0:
                if attrib.size() > 1:
                    values = values.reshape(-1,attrib.size())
                data['prims'][attrib.name()] = values.tolist()
